    ...
    <template v-if="!isFuture()">
        <div class="col-12">
            <entity-table
                ...
                :raw-processor="rawProcessor"
                :identifier="identifier"
                endpoint="findEvaluations"
                type="registration"
                :headers="headers"
                :phase="phase"
                :visible="['agent', 'number', 'result', 'status', 'evaluator']"
                :query="query"
                :limit="100"
                @clear-filters="clearFilters"
                @remove-filter="removeFilter($event)"
                :filtersDictComplement="filtersDictComplement">
                ...
                <template #searchKeyword='{query}'>
                    <textarea
                        ref="search"
                        v-model="this.query['registration:@keyword']"
                        rows="1"
                        placeholder="<?= i::__('Pesquisa por palavra-chave separados por ;') ?>"
                        class="entity-table__search-input">
                    </textarea>
                        ...
                        <div class="opportunity-evaluations-table__actions">
                            ...
                            <div v-if="user == 'all'">
                                <mc-export-spreadsheet
                                    ...
                                    :params="{entityType: 'registrationEvaluation', '@select': 'projectName,category,owner.{name},number,score,proponentType,range,eligible,user,result,status,evaluationData', query}"
                                    group="evaluations-spreadsheets">
                                </mc-export-spreadsheet>
                            </div>
                        </div>
                    </div>
                ...
                <template #filters="{entities,filters}">
                    <div class="opportunity-evaluations-table__filters grid-12">
                        <div v-if="hasControl" :class="hasControl ? 'col-3' : 'col-4'">
                            <mc-select
                                :options="evaluationsFiltersOptions"
                                v-model:default-value="evaluatiorFilter"
                                @change-option="filterByEvaluator($event, entities)"
                                placeholder="<?= i::__("Avaliador") ?>"
                                hide-filters>
                            </mc-select>
                        </div>
                        ...
                        <div :class="hasControl ? 'col-3' : 'col-4'">
                            <mc-select
                                :options="status"
                                v-model:default-value="selectedStatus"
                                @change-option="filterByStatus($event, entities)"
                                placeholder="<?= i::__("Estado da avaliação") ?>"
                                hide-filters>
                            </mc-select>
                        </div>
                            ...
                            <datepicker
                                ...
                                <template #dp-input="{ value, onBlur, onInput, onEnter, onTab, onClear }">
                                    <input
                                        ...
                                        :value="value"
                                        maxlength="10"
                                        @input="onChange($event, onInput, entities)"
                                        @blur="onBlur"
